CREATE DATABASE hotelENG;

USE hotelENG;

CREATE TABLE `Bookings` /*ZROBIONE*/
(
  ...
  `RoomID` int,
  `PaymentID` int,
  `Standard` enum('Normal', 'B&B', 'All-Inclusive', 'Half-Pension', 'Full-Pension'),
  `BooDate` date,
  `ArrDate` date,
  `DepDate` date,
  PRIMARY KEY (BookID)
...
CREATE TABLE `Payments` /*ZROBIONE*/
(
  `PaymentID` int auto_increment,
  `Total` float DEFAULT '100',
  `Way` enum('Karta', 'Gotowka', 'Przelew'),
  PRIMARY KEY (PaymentID)
);

CREATE TABLE `Employees` /*ZROBIONE*/
(
...
CREATE TABLE `ServInfo` /*ZROBIONE*/
(
  `code` enum('KAR','PAR', 'ROW','SPO','SUV','BEZ','NOR'),
  `price` int,
  `Descr` text,
  PRIMARY KEY (code)
);

CREATE TABLE `Services` /*ZROBIONE*/
(
  `BookID` int,
  `ServNb` int auto_increment,
  `code` enum('KAR','PAR','ROW','SPO','SUV','BEZ','NOR'),
  PRIMARY KEY (ServNb)
);
...
ALTER TABLE `Payments` ADD FOREIGN KEY (`PaymentID`) REFERENCES `Bookings` (`PaymentID`);

ALTER TABLE `Bookings` ADD FOREIGN KEY (`WorkID`) REFERENCES `Employees` (`WorkID`);

ALTER TABLE `Bookings` ADD FOREIGN KEY (`RoomID`) REFERENCES `Rooms` (`RoomID`);

ALTER TABLE `Services` ADD FOREIGN KEY (`code`) REFERENCES `ServInfo` (`code`);

ALTER TABLE `Bookings` ADD FOREIGN KEY (`BookID`) REFERENCES `Services` (`BookID`);
